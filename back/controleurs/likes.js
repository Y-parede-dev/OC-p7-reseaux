...
exports.like = (req, res, next) => {
    dataBase.query(`SELECT user_id FROM likes WHERE user_id=? AND post_id=?;`, [req.body.user_id, req.body.post_id], function(err, result){
        if(err){
            console.log(err);
        }else{
            if(result.length > 0){
                dataBase.query(`DELETE FROM likes WHERE user_id = ? AND post_id=?;`, [req.body.user_id, req.body.post_id],function (err, result){
                    if(err){
                        console.log(err);
                    }else{
